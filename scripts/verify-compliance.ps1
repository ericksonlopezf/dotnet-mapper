...
<#
.SYNOPSIS
    Automated zero-tolerance repository compliance & governance verifier for EricksonLopez.Mapper.
.DESCRIPTION
    Validates:
    ...
    5. Valid GitHub repository links referencing ericksonlopezf/dotnet-mapper.
...
if ($workflowFiles) {
    foreach ($wf in $workflowFiles) {
        $firstLine = (Get-Content $wf.FullName -TotalCount 1 -Encoding UTF8)
        if ($firstLine -notmatch "Copyright .* Erickson Lopez.* MIT License") {
...
if ($missingHeaders -eq 0) { Write-Host "  ✅ All C# files and workflow files contain the required canonical MIT copyright header." -ForegroundColor Green }
...
Write-Host "`n[5/8] Checking GitHub identity links (ericksonlopezf/dotnet-mapper)..." -ForegroundColor Yellow
$badLinks = 0
...
foreach ($f in $allTrackedFiles) {
    ...
    for ($i = 0; $i -lt $lines.Count; $i++) {
        if ($lines[$i] -match "github\.com/ericksonlopez/dotnet-mapper\b") {
...
if ($badLinks -eq 0) { Write-Host "  ✅ All GitHub URLs correctly target ericksonlopezf/dotnet-mapper." -ForegroundColor Green }
...
foreach ($proj in $projFiles) {
    ...
    for ($i = 0; $i -lt $lines.Count; $i++) {
        if ($lines[$i] -match "<NoWarn>.*(CS0618|CS0619|CS1591|CS0159|\bCS159\b|CA1305|CA1707).*</NoWarn>") {
...
if ($illegalNoWarn -eq 0) { Write-Host "  ✅ Zero prohibited NoWarn suppressions found (CS0618, CS0619, CS1591, CS159, CA1305, CA1707 fully enforced)." -ForegroundColor Green }

# 8. Package Metadata & Implicit Usings Audit
Write-Host "`n[8/8] Checking Package Metadata & Implicit Usings uniformity..." -ForegroundColor Yellow
$metaErrors = 0
$buildProps = Join-Path $RootDirectory "Directory.Build.props"
if (Test-Path $buildProps) {
    $bpContent = Get-Content $buildProps -Raw -Encoding UTF8
    if ($bpContent -notmatch "<PackageProjectUrl>https://ericksonlopez\.dev/mapper</PackageProjectUrl>") {
        Write-Host "  ❌ Invalid or outdated PackageProjectUrl in Directory.Build.props" -ForegroundColor Red
        $violations++
        $metaErrors++
    }
    if ($bpContent -notmatch "<ImplicitUsings>(enable|disable)</ImplicitUsings>") {
        Write-Host "  ❌ Missing centralized <ImplicitUsings> in Directory.Build.props" -ForegroundColor Red
        $violations++
        $metaErrors++
    }
    if ($bpContent -notmatch "<PackageLicenseExpression>MIT</PackageLicenseExpression>") {
        Write-Host "  ❌ Missing <PackageLicenseExpression>MIT</PackageLicenseExpression> in Directory.Build.props" -ForegroundColor Red
        $violations++
        $metaErrors++
    }
} else {
...
if ($metaErrors -eq 0) { Write-Host "  ✅ Package metadata and uniform Implicit Usings policy verified." -ForegroundColor Green }
...
# -----------------------------------------------------------------------------
# Markdown Internal Link Integrity Gate
# -----------------------------------------------------------------------------
Write-Host "`n[Gate: Markdown Link Integrity] Validating relative links in all markdown files..." -ForegroundColor Yellow
$linkErrors = 0
$mdFiles = Get-ChildItem -Path $targetRoot -Recurse -Filter "*.md" -File | Where-Object { $_.FullName -notmatch '[\\/](bin|obj|\.git|StrykerOutput|BenchmarkDotNet\.Artifacts)[\\/]' }
foreach ($doc in $mdFiles) {
    ...
    for ($i = 0; $i -lt $lines.Count; $i++) {
        ...
        foreach ($m in $matches) {
            ...
            if ($url -match '^(https?://|mailto:|#|file:|javascript:|data:)') { continue }
            
            $targetPath = $url.Split('#')[0].Trim()
            if ([string]::IsNullOrWhiteSpace($targetPath)) { continue }
            
            try {
                $combined = [System.IO.Path]::Combine($dir, $targetPath.Replace('/', [System.IO.Path]::DirectorySeparatorChar))
                $resolved = [System.IO.Path]::GetFullPath($combined)
                if (-not (Test-Path $resolved)) {
                    Write-Host "  ❌ Broken relative link in $($doc.FullName):$($i + 1) -> '$url'" -ForegroundColor Red
                    if (Get-Variable -Name "violations" -Scope 0 -ErrorAction SilentlyContinue) { $violations++ }
                    if (Get-Variable -Name "Violations" -Scope 0 -ErrorAction SilentlyContinue) { if ($Violations -is [System.Collections.IList]) { $Violations.Add("Broken relative link in $($doc.FullName):$($i + 1) -> '$url'") } else { $Violations++ } }
                    if (Get-Variable -Name "FailedChecks" -Scope 0 -ErrorAction SilentlyContinue) { $FailedChecks++ }
                    $linkErrors++
                }
            } catch {
                Write-Host "  ❌ Path resolution error in $($doc.FullName):$($i + 1) -> '$url' ($($_.Exception.Message))" -ForegroundColor Red
                if (Get-Variable -Name "violations" -Scope 0 -ErrorAction SilentlyContinue) { $violations++ }
                if (Get-Variable -Name "Violations" -Scope 0 -ErrorAction SilentlyContinue) { if ($Violations -is [System.Collections.IList]) { $Violations.Add("Path resolution error in $($doc.FullName):$($i + 1) -> '$url'") } else { $Violations++ } }
                if (Get-Variable -Name "FailedChecks" -Scope 0 -ErrorAction SilentlyContinue) { $FailedChecks++ }
                $linkErrors++
            }
        }
...
if ($linkErrors -eq 0) {
    Write-Host "  ✅ All internal markdown links resolved successfully." -ForegroundColor Green
}
