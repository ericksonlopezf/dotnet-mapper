...
";
        ...
        [Fact]
        public async Task CodeAction_WhenInvoked_ShouldAddMapIgnoreAttributeWithCorrectArguments()
        {
            var provider = new MapIgnoreCodeFixProvider();
            ...
            var actions = await CodeFixTestHelper.GetRegisteredCodeActionsAsync(
                ...
                descriptor: MockELM001PropertyAnalyzer.UnmappedMember,
                span: new TextSpan(77, 3),
                messageArgs: "IgnoreMe");

            actions.Should().ContainSingle();
            var action = actions[0];
            action.Title.Should().Be("Add [MapIgnore(\"IgnoreMe\")]");
            action.EquivalenceKey.Should().Be("MapIgnore_IgnoreMe");
            ...
            text.ToString().Should().Contain("[EricksonLopez.Mapper.MapIgnore(\"IgnoreMe\")]");
        }
    }
}
