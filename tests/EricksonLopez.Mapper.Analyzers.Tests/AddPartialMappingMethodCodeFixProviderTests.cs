...
";
    ...
    [Fact]
    public async Task CodeAction_WhenInvoked_ShouldGeneratePartialMethodWithExactSignatureAndParameters()
    {
        var provider = new AddPartialMappingMethodCodeFixProvider();
        string sourceText = "namespace TestNamespace; public partial class MyMapper { public partial SpecialDest Map(SpecialSource source); }";
        var actions = await CodeFixTestHelper.GetRegisteredCodeActionsAsync(
            provider,
            sourceText: sourceText,
            descriptor: MockELM003Analyzer.UnsupportedConversion,
            span: new TextSpan(85, 3),
            messageArgs: new object[] { "SpecialSource", "SpecialDest", "Value" });

        actions.Should().ContainSingle();
        var action = actions[0];
        action.Title.Should().Be("Add partial mapping method for SpecialSource → SpecialDest");
        action.EquivalenceKey.Should().Be("AddPartialMapping_SpecialSource_SpecialDest");
        ...
        code.Should().Contain("public partial SpecialDest MapSpecialDest(SpecialSource source);");

        var root = await changedDoc.GetSyntaxRootAsync();
        var generatedMethod = root!.DescendantNodes().OfType<MethodDeclarationSyntax>().First(m => m.Identifier.Text == "MapSpecialDest");
        generatedMethod.ReturnType.ToString().Should().Be("SpecialDest");
        generatedMethod.Identifier.Text.Should().Be("MapSpecialDest");
        generatedMethod.ParameterList.Parameters.Should().ContainSingle();
        generatedMethod.ParameterList.Parameters[0].Identifier.Text.Should().Be("source");
        generatedMethod.ParameterList.Parameters[0].Type!.ToString().Should().Be("SpecialSource");
    }

    [Fact]
    public async Task CodeAction_WhenInvokedWithNamespaceQualifiedTypes_ShouldExtractSimpleNamesAndGenerateCorrectSignature()
    {
        var provider = new AddPartialMappingMethodCodeFixProvider();
        string sourceText = "namespace TestNamespace; public partial class MyMapper { public partial Other.SpecialDest Map(My.SpecialSource source); }";
        var actions = await CodeFixTestHelper.GetRegisteredCodeActionsAsync(
            provider,
            sourceText: sourceText,
            descriptor: MockELM003Analyzer.UnsupportedConversion,
            span: new TextSpan(90, 3),
            messageArgs: new object[] { "My.SpecialSource", "Other.SpecialDest", "Value" });
    ...
    [Fact]
    public void ExtractTypes_WhenValidMessageProvided_ShouldReturnExtractedComponents()
    {
        var message = "Cannot convert from 'Full.Namespace.SourceType' to 'Target.Namespace.DestType' for member 'MemberProp'";
        var result = AddPartialMappingMethodCodeFixProvider.ExtractTypes(message);

        result.Should().NotBeNull();
        result!.Value.sourceType.Should().Be("SourceType");
        result.Value.destType.Should().Be("DestType");
        result.Value.memberName.Should().Be("MemberProp");
    }

    [Fact]
    public void ExtractTypes_WhenSimpleTypesProvided_ShouldReturnExactTypeNames()
    {
        var message = "Cannot convert from 'Src' to 'Dst' for member 'Prop'";
        var result = AddPartialMappingMethodCodeFixProvider.ExtractTypes(message);

        result.Should().NotBeNull();
        result!.Value.sourceType.Should().Be("Src");
        result.Value.destType.Should().Be("Dst");
        result.Value.memberName.Should().Be("Prop");
    }
}
