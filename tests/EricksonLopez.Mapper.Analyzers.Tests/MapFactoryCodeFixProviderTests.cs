...
";
    ...
    [Fact]
    public async Task CodeAction_WhenInvoked_ShouldAddMapFactoryAttributeWithCorrectArguments()
    {
        var provider = new MapFactoryCodeFixProvider();
        ...
        action.Title.Should().Be("Add [MapFactory(\"Create\")]");
        action.EquivalenceKey.Should().Be("MapFactory_Create");
        ...
        text.ToString().Should().Contain("[EricksonLopez.Mapper.MapFactory(\"Create\")]");
    }
}
