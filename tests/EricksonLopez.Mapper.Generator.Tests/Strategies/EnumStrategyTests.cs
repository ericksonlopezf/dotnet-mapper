...
";
    ...
    [Fact]
    public void MapEnumValue_WhenStandaloneMethodWithIgnoreCase_ShouldEmitExpectedSwitchArms()
    {
        string source = @"
public enum SourceMode { fast, slow, custom }
public enum TargetMode { Fast, Slow, Standard }

[Mapper]
public partial class ModeMapper
{
    [EnumMappingStrategy(EnumMappingStrategy.ByName, IgnoreCase = true)]
    [MapEnumValue(SourceMode.custom, TargetMode.Standard)]
    public partial TargetMode MapMode(SourceMode source);
}
";
        var (diagnostics, output, _) = GeneratorTestHelper.RunGenerator(source, verifyEmittedCodeCompiles: true);

        diagnostics.Where(d => d.Severity == DiagnosticSeverity.Error).Should().BeEmpty();
        output.Should().Contain("SourceMode.custom => global::TestNamespace.TargetMode.Standard");
        output.Should().Contain("SourceMode.fast => global::TestNamespace.TargetMode.Fast");
        output.Should().Contain("SourceMode.slow => global::TestNamespace.TargetMode.Slow");
    }

    [Theory]
    [InlineData(true)]
    [InlineData(false)]
    public void MapEnumValue_WhenMultipleExplicitRemappings_ShouldGenerateAllExplicitMappings(bool strictMapping)
    {
        string strictFlag = strictMapping ? "true" : "false";
        string source = $@"
public enum SourceAction {{ Create, Update, Delete, Unknown }}
public enum TargetAction {{ Insert, Modify, Remove, DefaultAction }}

public class Source {{ public SourceAction Action {{ get; set; }} }}
public class Target {{ public TargetAction Action {{ get; set; }} }}

[Mapper(StrictMapping = {strictFlag})]
public partial class ActionMapper
{{
    [MapEnumValue(SourceAction.Create, TargetAction.Insert)]
    [MapEnumValue(SourceAction.Update, TargetAction.Modify)]
    [MapEnumValue(SourceAction.Delete, TargetAction.Remove)]
    [MapEnumValue(SourceAction.Unknown, TargetAction.DefaultAction)]
    public partial Target Map(Source source);
}}
";
        var (diagnostics, output, _) = GeneratorTestHelper.RunGenerator(source, verifyEmittedCodeCompiles: true);
        ...
        diagnostics.Where(d => d.Severity == DiagnosticSeverity.Error).Should().BeEmpty();
        output.Should().Contain("SourceAction.Create => global::TestNamespace.TargetAction.Insert");
        output.Should().Contain("SourceAction.Update => global::TestNamespace.TargetAction.Modify");
        output.Should().Contain("SourceAction.Delete => global::TestNamespace.TargetAction.Remove");
        output.Should().Contain("SourceAction.Unknown => global::TestNamespace.TargetAction.DefaultAction");
    }
}
