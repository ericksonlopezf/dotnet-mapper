...
public class CodeEmitterTests
{
    ...
    [Fact]
    public void EscapeIdentifier_WhenEmptyOrDigitFirst_ShouldPrependUnderscore()
    {
        CodeEmitter.EscapeIdentifier("").Should().Be("_");
        CodeEmitter.EscapeIdentifier(null!).Should().Be("_");
        CodeEmitter.EscapeIdentifier("   ").Should().Be("_");
        CodeEmitter.EscapeIdentifier("123Number").Should().Be("_123Number");
        CodeEmitter.EscapeIdentifier("1Foo").Should().Be("_1Foo");
    }

    [Fact]
    public void GenerateSourceCode_WhenEnumerableToImmutableArrayWithoutCount_ShouldEmitTryGetNonEnumeratedCount()
    {
        var immArrayStrategy = new ConversionStrategy.EnumerableMapping(new ConversionStrategy.DirectAssignment(), "int", "long", IsArray: false, IsList: false, IsImmutableArray: true, SourceIsArray: false, SourceHasCount: false);
        var method = TestDataBuilders.CreateMethod("Map")
            .WithSourceType("global::System.Collections.Generic.IEnumerable<int>")
            .WithTargetType("global::System.Collections.Immutable.ImmutableArray<long>")
            .AddMember("Items", "Items", immArrayStrategy)
            .Build();

        var typeMapping = TestDataBuilders.CreateType("TestMapper")
        ...
        code.Should().Contain("TryGetNonEnumeratedCount");
        code.Should().Contain("CreateBuilder<long>(_c");
    }

    [Fact]
    public void GenerateSourceCode_WhenDictionaryMappingWithoutCount_ShouldEmitTryGetNonEnumeratedCount()
    {
        var dictStrategy = new ConversionStrategy.DictionaryMapping(new ConversionStrategy.DirectAssignment(), new ConversionStrategy.DirectAssignment(), "string", "string", "int", "int", SourceHasCount: false);
        var method = TestDataBuilders.CreateMethod("Map")
            .WithSourceType("global::System.Collections.Generic.IEnumerable<global::System.Collections.Generic.KeyValuePair<string, int>>")
            .WithTargetType("global::System.Collections.Generic.Dictionary<string, int>")
            .AddMember("Dict", "Dict", dictStrategy)
            .Build();

        var typeMapping = TestDataBuilders.CreateType("DictMapper")
            .WithNamespace("TestNs")
            .WithStatic(false)
            .AddMethod(method)
            .Build();
        ...
        var code = CodeEmitter.GenerateSourceCode(typeMapping);
        code.Should().Contain("TryGetNonEnumeratedCount");
        code.Should().Contain("new global::System.Collections.Generic.Dictionary<string, int>(_dc");
    }

    [Fact]
    public void GenerateSourceCode_WhenCustomValueExpression_ShouldEmitCustomExpression()
    {
        var method = TestDataBuilders.CreateMethod("Map")
            .WithSourceType("Source")
            .WithTargetType("Dest")
            .AddMember(new MemberMapping("Field", "Field", new ConversionStrategy.DirectAssignment(), CustomValueExpression: "\"CustomStaticVal\""))
            .Build();

        var typeMapping = TestDataBuilders.CreateType("ValMapper")
            .WithNamespace("TestNs")
            .WithStatic(false)
            .AddMethod(method)
            .Build();
        ...
        var code = CodeEmitter.GenerateSourceCode(typeMapping);
        code.Should().Contain("Field = \"CustomStaticVal\"");
    }
    ...
    [Fact]
    public void GenerateSourceCode_WhenStaticMapperAndMapMethodInvocation_ShouldEmitStaticMethodCallWithoutThis()
    {
        var method = TestDataBuilders.CreateMethod("Map")
            .WithSourceType("Source")
            .WithTargetType("Dest")
            .AddMember("Nested", "Nested", new ConversionStrategy.MapMethodInvocation("MapNested", IsSourceNullable: false, IsTargetNullable: false, MethodKey: "MapNested(NestedSource)"))
            .Build();
        ...
        var code = CodeEmitter.GenerateSourceCode(typeMapping);
        code.Should().Contain("MapNested(source.Nested)");
        code.Should().NotContain("this.MapNested");
    }

    [Fact]
    public void GenerateSourceCode_WhenStaticMapperAndNullableMapMethodInvocation_ShouldEmitTernaryNullCheckWithoutThis()
    {
        var method = TestDataBuilders.CreateMethod("Map")
            .WithSourceType("Source")
            .WithTargetType("Dest")
            .AddMember("Nested", "Nested", new ConversionStrategy.MapMethodInvocation("MapNested", IsSourceNullable: true, IsTargetNullable: true, MethodKey: "MapNested(NestedSource?)"))
            .Build();
        ...
        var code = CodeEmitter.GenerateSourceCode(typeMapping);
        code.Should().Contain("(source.Nested != null ? MapNested(source.Nested) : null)");
        code.Should().NotContain("this.MapNested");
    }
