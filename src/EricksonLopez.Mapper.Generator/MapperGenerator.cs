...
public class MapperGenerator : IIncrementalGenerator
{
    ...
    public void Initialize(IncrementalGeneratorInitializationContext context)
    {
        ...
        var assemblyHasDiAttrProvider = context.SyntaxProvider
            .ForAttributeWithMetadataName(
                "EricksonLopez.Mapper.GenerateMapperRegistrationAttribute",
                predicate: (node, _) => true,
                transform: (ctx, _) => true)
            .Collect()
            .Select((coll, _) => coll.Length > 0);
    ...
    internal static Models.TypeMapping? ProcessMapperSymbol(INamedTypeSymbol mapperSymbol, Compilation compilation, CancellationToken cancellationToken)
    {
        ...
        if (assemblyDefaults != null)
        {
            ...
            {
                if (namedArg.Key == "StrictMapping" && namedArg.Value.Value is bool s) isStrict = s;
                else if (namedArg.Key == "EnumMappingStrategy" && namedArg.Value.Value is int es) defaultEnumStrategy = es;
                else if (namedArg.Key == "EnumIgnoreCase" && namedArg.Value.Value is bool eic) defaultEnumIgnoreCase = eic;
            }
        }
        ...
        {
            if (classEnumAttr.ConstructorArguments.Length == 1 && classEnumAttr.ConstructorArguments[0].Value is int csVal)
            {
            ...
            foreach (var attr in methodSymbol.GetAttributes())
            {
                ...
                else if (attrName is "MapIgnoreAttribute")
                {
                    if (attr.ConstructorArguments.Length == 1 &&
                        attr.ConstructorArguments[0].Value is string dst)
                    {
                ...
                {
                    if (attr.ConstructorArguments.Length == 1 && attr.ConstructorArguments[0].Value is int sVal)
                    {
                ...
                {
                    if (attr.ConstructorArguments.Length == 2 &&
                        attr.ConstructorArguments[0].Value is ITypeSymbol srcType &&
                ...
                else if (attrName is "UseConverterAttribute")
                {
                    if (attr.ConstructorArguments.Length == 1)
                    {
                        if (attr.ConstructorArguments[0].Value is ITypeSymbol converterType)
                        {
                            ...
                            {
                                var location = attr.ApplicationSyntaxReference?.GetSyntax().GetLocation() ?? RoslynInvariants.GetLocation(methodSymbol);
