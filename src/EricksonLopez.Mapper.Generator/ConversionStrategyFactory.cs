...
internal static class ConversionStrategyFactory
{
        ...
        Dictionary<string, string>? explicitEnumValues = null)
    {
        ...
        if (unwrappedTarget is INamedTypeSymbol ntt)
        {
            ...
            {
                bool isStruct = ntt.IsValueType;
                bool isRecord = ntt.IsRecord;
                bool isReadOnly = ntt.IsReadOnly;
                bool hasValueProp = properties[0].Name == "Value";
                bool hasAttribute = ntt.GetAttributes().Any(a =>
                {
                    var name = RoslynInvariants.GetAttributeClassName(a);
                    return name is "ValueObjectAttribute" or "ValueObject";
                });

                if (hasAttribute || hasValueProp || (isStruct && isRecord && isReadOnly))
                {
        ...
        if (unwrappedSource is INamedTypeSymbol nst)
        {
            ...
            {
                bool isStruct = nst.IsValueType;
                bool isRecord = nst.IsRecord;
                bool isReadOnly = nst.IsReadOnly;
                bool hasValueProp = properties[0].Name == "Value";
                bool hasAttribute = nst.GetAttributes().Any(a =>
                {
                    var name = RoslynInvariants.GetAttributeClassName(a);
                    return name is "ValueObjectAttribute" or "ValueObject";
                });
